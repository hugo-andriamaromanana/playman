...
def mkdir_user(username):
    os.mkdir(os.path.join(os.path.dirname(__file__),'..', '..', 'users', username))
    print(f'User: "{username}" created successfully')
    print(
        f'Path: {os.path.join(os.path.dirname(__file__),"..","..","users",username)}')


def init_items_in_user_DIR(username):
    items_path = os.path.join(os.path.dirname(
        __file__),"..", '..', 'users', username, 'items.csv')
